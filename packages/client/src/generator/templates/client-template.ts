...
export function generateClientTemplate(
  ...
  const importList = importItems.map((item) => `  ${item}`).join(',\n');
...
export const api = createApiClient(${
    useEnhancedLogger || useCancellation || useDebounce
      ? `undefined, undefined, {
  ${useEnhancedLogger ? `errorInterceptors: [createDefaultErrorInterceptor()],` : ''}
  ${
    useCancellation || useDebounce
      ? `// Configure HTTP client with advanced features
  httpClientOptions: {
    ${
      useCancellation
        ? `cancellation: {
